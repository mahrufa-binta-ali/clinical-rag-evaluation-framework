---
title: Clinical RAG Evaluation Framework
emoji: 🧬
colorFrom: blue
colorTo: indigo
sdk: docker
...
2. `clinical_rag_eval.ingest` reads PDFs with `pypdf`.
...
6. Chunks are embedded with `BAAI/bge-small-en-v1.5` by default.
7. Embeddings, text, and metadata are persisted in ChromaDB under `chroma_db/`.
8. `clinical_rag_eval.query` embeds a user question and prints the top retrieved chunks with source metadata.
...
```bash
python -m clinical_rag_eval.ingest
```

Optional arguments:

```bash
python -m clinical_rag_eval.ingest --data-dir data --persist-dir chroma_db
```

`python -m clinical_rag_eval.ingest` rebuilds the Chroma collection from scratch by default. Old chunks are deleted before the current PDFs are embedded, which prevents stale results after deleting or renaming files. After ingestion, the script prints the unique source filenames stored in the collection.
...
```bash
python -m clinical_rag_eval.ingest --chunking-method char
```

The default chunking settings live in `src/clinical_rag_eval/config.py`:
...
```bash
python -m clinical_rag_eval.query
```
...
```bash
python -m clinical_rag_eval.query "What does the document say about anticoagulation?"
```

Set the number of results:

```bash
python -m clinical_rag_eval.query "How should renal function be monitored?" --top-k 5
```

Use optional cross-encoder reranking:

```bash
python -m clinical_rag_eval.query "How should renal function be monitored?" --top-k 5 --rerank --candidate-k 10
```

You can also set `--top-k` for interactive mode:

```bash
python -m clinical_rag_eval.query --top-k 5
```

Use reranking in interactive mode:

```bash
python -m clinical_rag_eval.query --top-k 5 --rerank --candidate-k 10
```

Control chunk preview length:

```bash
python -m clinical_rag_eval.query "What is retrieval augmented generation?" --preview-chars 1200
```
...
The deployed demo exposes the API layer of the project. `GET /` opens a simple hosted landing page, `/docs` provides interactive API documentation, and `/health` is mainly an API status endpoint. The `/query` endpoint requires a populated local ChromaDB vector store, so it may not return retrieval results on the hosted demo unless documents and vector indexes are available inside the deployment environment.
...
```bash
python -m clinical_rag_eval.evaluate
```

Set the number of retrieved chunks used for each evaluation query:

```bash
python -m clinical_rag_eval.evaluate --top-k 5
```

Evaluate with cross-encoder reranking:

```bash
python -m clinical_rag_eval.evaluate --rerank
```

You can also control candidate depth:

```bash
python -m clinical_rag_eval.evaluate --rerank --candidate-k 10
```
